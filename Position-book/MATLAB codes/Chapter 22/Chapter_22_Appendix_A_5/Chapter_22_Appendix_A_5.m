...
% Kalman filter Parameters
  lever_arm_XYZ_body = [0;0;0]; %[m]
  std_white_gyro_noise = [0.96, 0.42, 0.81]; %deg/sqrt(hr)
  std_white_acc_noise =  [4*1e-6,4*1e-6,6*1e-6]; %G/sqrt(hr)
  std_GM_gyro_noise = [6.7, 5.6 , 4.2];  %deg/hr
  std_GM_acc_noise = [41.7*1e-6, 53.8*1e-6 , 61.4*1e-6];  %G/hr
  tau_acc = [0.047;0.021;0.046]; % [hr]
  tau_gyro = [0.048;0.021;0.046]; % [hr]
  Num_states  = 15;
 ...
 P(11,11) = (4e-4*g)^2;
 P(12,12) = (4e-4*g)^2;
 P(13,13)=  ((50*pi/180)/3600)^2;
 P(14,14)=  ((50*pi/180)/3600)^2;
 P(15,15)=  ((50*pi/180)/3600)^2;
                 
  navdata.bias_acc          = zeros(3,floor(n_samples*(1/PVT_rate)));              
  navdata.bias_gyros        = zeros(3,floor(n_samples*(1/PVT_rate)));  
  iter_low_rate = 1;
  iteration_GPS = 1;
...
for sample_idx=2:n_samples
         %% medium_rate part coning and sculling computation, Euler Angle computation, position and velocity estimation
            ...
            [angle_b_frame,delta_angle_b_frame, navdata.coning(:,sample_idx), vel_b_frame,delta_vel_b_frame, navdata.sculling(:,sample_idx)]= coning_sculling_high_rate(...
                                                                                            [acc_x_filtered(delay+trans+sample_idx-1);acc_y_filtered(delay+trans+sample_idx-1);acc_z_filtered(delay+trans+sample_idx-1)].*g-Det_bias_acc-navdata.bias_acc(:,iteration_INS-1),... 
                                                                                            [gyro_x_filtered(delay+trans+sample_idx-1);gyro_y_filtered(delay+trans+sample_idx-1);gyro_z_filtered(delay+trans+sample_idx-1)].*pi/180 - Det_bias_gyros*pi/180 -navdata.bias_gyros(:,iteration_INS-1),...
                                                                                            vel_b_frame ,delta_vel_b_frame,angle_b_frame,delta_angle_b_frame,navdata.coning(:,sample_idx-1),... 
            ...
            if iter_high_rate == iter_medium_rate 
                       ...
                       iter_low_rate = iter_low_rate + 1;
                        
                       
                       %% KALMAN FILTER PREDICTION
                       %% Compute the Phi Transition Matrix for the KF

                         phi     = computeF_ContinuousTIME(navdata.r_e(:,iteration_INS)+navdata.R_b_e(:,:,iteration_INS)*lever_arm_XYZ_body,....
                                              navdata.R_b_e(:,:,iteration_INS)*([acc_x_filtered(delay+trans+sample_idx-1);acc_y_filtered(delay+trans+sample_idx-1);acc_z_filtered(delay+trans+sample_idx-1)].*g-Det_bias_acc-navdata.bias_acc(:,iteration_INS-1)),...
                                              navdata.R_b_e(:,:,iteration_INS),tau_acc, tau_gyro, Num_states );
                        
                        FDisc   = FDiscrete_ECEF(dt_medium_rate,phi);
                       
                         %% Compute the Noise Matrix of the Model 
                         G       =   GMatrix2_Loosely_ECEF(navdata.R_b_e(:,:,iteration_INS));
                         QDisc   =   QDiscrete_Loosely_ECEF(dt_medium_rate,FDisc,G, std_white_gyro_noise, std_white_acc_noise,...
                                                           std_GM_gyro_noise,std_GM_acc_noise, tau_gyro,tau_acc );% JTM 3/3/11 
                                                       
                                                                                                            
                        %% prediction:   we predict only the Bias                                  
                         x_predict=FDisc(10:15,10:15)*[navdata.bias_acc(:,iteration_INS-1);navdata.bias_gyros(:,iteration_INS-1)];
                         navdata.bias_acc(:,iteration_INS) = x_predict(1:3);
                         navdata.bias_gyros(:,iteration_INS) = x_predict(4:6);
                         P = FDisc * P *(FDisc') + QDisc; 
                         
                         if(iter_low_rate == PVT_rate)
                                 %% GPS Measurements UPDATE

                                 % INS pos and vel correction by taking into account the lever arm
                                 INSpos = navdata.r_e(1:3,iteration_INS)+navdata.R_b_e(:,:,iteration_INS)*lever_arm_XYZ_body;
                                 INSvel = navdata.v_e(1:3,iteration_INS)+(skew([0;0;Const.WE])*navdata.R_b_e(:,:,iteration_INS)+ navdata.R_b_e(:,:,iteration_INS)*skew(angle_b_frame))*lever_arm_XYZ_body;
            
                                 % GPS position
                                 GPSpos = [PosStart.Xu;PosStart.Yu;PosStart.Zu];
                                 GPSvel = [PosStart.Vx;PosStart.Vy;PosStart.Vz];
                                 
                                 disp([ num2str(iteration_INS) ' calculating loosely-coupled solution (sample_idx ' num2str(sample_idx) ')']);
                                 KalmanStates = LooselyCoupling_ECEF_ASSIST(INSpos,GPSpos, INSvel, GPSvel, Num_states,P,QDisc);
                                 ...
                                 X = KalmanStates.X;
                                 P = KalmanStates.P;
                                 navdata.Pxx_OK(:,iteration_GPS) = diag(P); % JTM 3/3/11
                                 navdata.X_OK(:,iteration_GPS) = X;
                                 
                                 navdata.r_e(:,iteration_INS)     =  navdata.r_e(:,iteration_INS)  +X(1:3);
                                 navdata.v_e(:,iteration_INS)     =  navdata.v_e(:,iteration_INS)  +X(4:6);
                                 epsilon                         =  [X(7);X(8);X(9)];
                                 navdata.epsilon(:,iteration_INS) =   epsilon;
                                 epsilon_cross = skew(epsilon); 
                                 
                                 R_b_eToTest = (eye(3)+epsilon_cross) *  navdata.R_b_e(:,:,iteration_INS);
                                 navdata.R_b_e(:,:,iteration_INS) = R_b_eToTest;
                                 navdata.R_b_n(:,:,iteration_INS) = navdata.R_n_e(:,:,iteration_INS).'*navdata.R_b_e(:,:,iteration_INS);
                                 R_b_n_corrected = navdata.R_b_n(:,:,iteration_INS);
                                 Euler_corrected = DCM2Euler( R_b_n_corrected );
                                 navdata.Euler(:,iteration_INS) = [Euler_corrected.roll,Euler_corrected.pitch,Euler_corrected.yaw];
                                 % update the estimates of the bias
                                 navdata.bias_acc(:,iteration_INS) = navdata.bias_acc(:,iteration_INS)- X(10:12);
                                 navdata.bias_gyros(:,iteration_INS) = navdata.bias_gyros(:,iteration_INS)- X(13:15);
                                 
                                 iteration_GPS = iteration_GPS+1;
                         end
