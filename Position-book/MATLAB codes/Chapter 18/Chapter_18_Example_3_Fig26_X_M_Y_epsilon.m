% RMSE of the position estimate as a function of the number of the NLOS BSs
...
for M = M_range;
    ...
    for scenario = 1  %scenario_range
        ...
        for n_R = 1 : N_R
            ...
            SE_p_LS(n_M,n_R) = sum((hat_p_LS - p_true).^2);
            SE_p_WLS_known_gamma(n_M,n_R) = sum((hat_p_WLS_known_gamma - p_true).^2);
            SE_p_ML_known_gamma(n_M,n_R) = sum((hat_p_ML_known_gamma - p_true).^2);
        ...
        RMSE_p_LS(n_M) = sqrt(mean(SE_p_LS(n_M,:)))
        RMSE_p_WLS_known_gamma(n_M) = sqrt(mean(SE_p_WLS_known_gamma(n_M,:)))
        B_LS_theory = c^2 * inv(Phi_bar * Phi_bar') * Phi_bar * diag( sigma2_true(M+1:B) ) * Phi_bar' * inv(Phi_bar * Phi_bar');
        RMSE_p_LS_theory(n_M) = sqrt( trace( B_LS_theory ) );
        RMSE_p_ML_known_gamma(n_M) = sqrt(mean(SE_p_ML_known_gamma(n_M,:)))
        ...
        bias_p_LS(n_M) = mean(sqrt( error_p_LS(1,:).^2 + error_p_LS(2,:).^2 ));
        bias_p_WLS_known_gamma(n_M) =  mean(sqrt(error_p_WLS_known_gamma(1,:).^2 + error_p_WLS_known_gamma(2,:).^2 ));
        bias_p_ML_known_gamma(n_M) = mean(sqrt( error_p_ML_known_gamma(1,:).^2 + error_p_ML_known_gamma(2,:).^2 ));
        CRB_p_given_gamma(n_M) = sqrt(trace(B_pp_given_gamma)); 
    end
end
 ...
 %subplot(2,1,1);
 grid on
 plot(M_range,RMSE_p_LS,'r*','LineWidth',.5,'MarkerEdgeColor','r','MarkerSize',5)
 plot(M_range,RMSE_p_LS_theory,':r','LineWidth',.5,'MarkerEdgeColor','r','MarkerSize',5)
 plot(M_range,RMSE_p_WLS_known_gamma,'mo','LineWidth',.5,'MarkerEdgeColor','m','MarkerSize',5)
 plot(M_range,RMSE_p_ML_known_gamma,'bs','LineWidth',.5,'MarkerEdgeColor','b','MarkerSize',5)
 plot(M_range,CRB_p_given_gamma,'-k','LineWidth',.5,'MarkerEdgeColor','k','MarkerSize',5)
 ...
 %subplot(2,1,2);
 grid on
 plot(M_range,bias_p_LS,'r*','LineWidth',.5,'MarkerEdgeColor','r','MarkerSize',5)
 plot(M_range,bias_p_WLS_known_gamma,'mo','LineWidth',.5,'MarkerEdgeColor','m','MarkerSize',5)
 plot(M_range,bias_p_ML_known_gamma,'bs','LineWidth',.5,'MarkerEdgeColor','b','MarkerSize',5)
