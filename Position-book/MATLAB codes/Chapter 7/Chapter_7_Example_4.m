...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  EXAMPLE 7.4: IFFT based TOA estimation method
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% we assume the length of channel is equal to length of transmit signal.
% the channel impulse response estimated by Inverse Fourier Transform(IFFT)
% as a realization of deconvolution method. 
% we use a threshold to Estimate TOA from estimated CIR
%  This threshold can be selected based on the measured noise power. 
%------------------------------------------------------------------------
...
fft_rx_sig=fft(rx_sig_matrix(:,1),sample_nom);
                     % estimate the channel frequency response (CFR)
h_f=fft_rx_sig ./ tx_symbol_freq_domain;
                     % estimated estimated channel impulse response
h_estimated_t=ifft(h_f,sample_nom);
...
ray_index_deconv=find(abs(h_estimated_t)  > component_noise_sigma*20);
if ~isempty(ray_index_deconv)
   deconv_estimated_TOA=ray_index_deconv(1)*ts
else
   deconv_estimated_TOA=NaN
end 
figure
plot(ts*(1:length(h_estimated_t)),abs(h_estimated_t),'k');
hold on;
plot(ts*(1:length(h_estimated_t)),...
            component_noise_sigma*10*ones(size(h_estimated_t)),'k-.');
xlabel('nSec');
title('Estimated Channel Impulse Response after Inverse Fourier');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
