...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% EXAMPLE 7.5:  MUSIC based TOA estimation techniques
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% uses Signal Processing Toolbox
%------------------------------------------------------------------------
% At the first step, the received data is converted to frequency domain.
% then the transmitted signal is removed and the channel frequency response (CFR) is estimated,
% we apply MUSIC to form the Pesudo-spectrum of received signal.
% here we use pmusic command from signal processing Toolbox
% In this example, we assume the number of paths is equal to the assumed maximum channel length.
%------------------------------------------------------------------------
...
fft_rx_matrix=fft(rx_sig_matrix,sample_nom);
                     % estimate the channel frequency response (CFR)
h_f_matrix = fft_rx_matrix ./ tx_matrix;
h_f_matrix=[h_f_matrix(sample_nom/2+1:end,:);
            h_f_matrix(1:sample_nom/2,:)];
                     % estimate the psudoscpectrum of MUSIC method
[music_pseudospectrum b]=pmusic(conj(h_f_matrix(:)),sample_nom);
                     % Compare the absolute of CIR with a cetain threshold to detect first received ray
                     % this threshold can be selected based on the noise power
ray_index_music=find(music_pseudospectrum  > component_noise_sigma*9e5);
if ~isempty(ray_index_music)
   music_estimated_TOA=b(ray_index_music(1))*ts*sample_nom/(2*pi)
else
   music_estimated_TOA=NaN
end 
figure;
plot(b*ts*sample_nom/(2*pi),music_pseudospectrum,'k');
hold on;
plot(b*ts*sample_nom/(2*pi),component_noise_sigma*4e5*ones(size(b)),'k-.');
xlabel('nSec');
ylabel('MUSIC Pesudospectrum');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
