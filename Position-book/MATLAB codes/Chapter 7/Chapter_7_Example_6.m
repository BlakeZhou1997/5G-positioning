...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% EXAMPLE 7.6:  Contiguous multi-band concatenation
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% In this example, we implement multi-band scenario with three sub band, 
% Correspond to each band, we have one CIR generated in example 7.1.
% we concatenate these channels in frequency domain and extract  the CIR correspond to full band.
%------------------------------------------------------------------------
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                      % Generate transmit signal for multiband scenario
multiband_tx=repmat(tx_symbol_freq_domain,1,length(fc));
                      % Here, we cnsider equal sample for input and output
multiband_fft_in=[multiband_rx_sig; zeros(size(h_b,1),length(fc))];
multiband_fft_in(1:size(h_b,1),:)=multiband_fft_in(1:size(h_b,1),:)+...
              multiband_fft_in(sample_nom+1:sample_nom+size(h_b,1),:);
multiband_fft_in=multiband_fft_in(1:sample_nom,:);
multiband_fft_out=fft(multiband_fft_in,sample_nom);
                     % estimate the CFR for each band
multiband_h_f = multiband_fft_out ./ multiband_tx;
                     % concatinate different subband in one vector
multiband_h_f=[multiband_h_f(sample_nom/2+1:end,:);
               multiband_h_f(1:sample_nom/2,:)];
multiband_h_f=multiband_h_f(:);
multiband_h_f=[multiband_h_f(length(multiband_h_f)/2+1:end); 
               multiband_h_f(1:length(multiband_h_f)/2)];
                    % estimate thr CIR from concatinated vector
equivalent_h_t=ifft(multiband_h_f);
figure;
plot(ts*(1:channel_length),abs(h_b(:,1)),'k-.');
hold on;
plot(ts*(1:length(equivalent_h_t))/length(fc),abs(equivalent_h_t),'k');
xlabel('nSec');
title('channel impulse response');
legend('Single Band', 'Multi Band');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
